<script>
$();
$(document).ready(function(){$("#jdvtv-list tr").hide();var highlight=function(string){$("#jdvtv-list .tr match").each(function(){var matchStart=$(this).text().toLowerCase().indexOf(""+string.toLowerCase()+"");var matchEnd=matchStart+string.length-1;var beforeMatch=$(this).text().slice(0,matchStart);var matchText=$(this).text().slice(matchStart,matchEnd+1);var afterMatch=$(this).text().slice(matchEnd+1);$(this).html(beforeMatch+"<em>"+matchText+"</em>"+afterMatch)})};$("#search-jdvtv").on("keyup click input",function(){if(this.value.length>0){$("#jdvtv-list tr").removeClass("match").hide().filter(function(){return $(this).text().toLowerCase().indexOf($("#search-jdvtv").val().toLowerCase())!=-1}).addClass("match").show();highlight(this.value);$("#jdvtv-list").show()}else{$("#jdvtv-list, #jdvtv-list tr").removeClass("match").hide()}})});function myFunction(){document.getElementById("myDropdown").classList.toggle("show")}
